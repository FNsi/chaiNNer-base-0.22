...
class AC_MSA(nn.Module):
    ...
    def forward(self, qkv, sim, x_size):
        ...
        shuffled_qkv = shuffled_qkv.to('cpu')
        pad_n = ng * gs - n
        fliped_qkv = torch.flip(shuffled_qkv[:, n - pad_n : n, :], dims=[1])
        paded_qkv = torch.cat(
            (shuffled_qkv, fliped_qkv),
            dim=1,
        )
        y = paded_qkv.reshape(b, -1, gs, c3).to('privateuseone')
